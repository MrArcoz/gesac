...
namespace GESAC
{
    public class EstadosRepository : IEstadosRepository
    {
        ...
        public IEnumerable<Estados> consulta()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    ID = Convert.ToInt16(dr["IdEstado"].ToString()),
        ...
        public bool estatus(int id)
        {
            ...
            using (IDatabaseFactory factory = new DatabaseFactory(cadena_cn))
            {
                ...
                try
                {
                    bd.executeNonQuery("uspEstados", new SqlParameter("@Opcion", 2), new SqlParameter("@ID", id));
                }
                catch (Exception ex)
                {
                    unitofwork.Rollback();
